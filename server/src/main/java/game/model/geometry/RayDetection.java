...
public class RayDetection {
    ...
    public static ArrayList<Point> getVertexModel(BufferedImage image, int axes, Point pivot){
        Polygon bound = new Polygon(
                new int[]{1, 1, image.getWidth(), image.getWidth()},
                new int[]{1, image.getHeight(), image.getHeight(), 1}
                , 4);
    ...
    public static ArrayList<Point> getVertexModel(BufferedImage image, int axes, Point pivot, Polygon bound){
        ...
        double xOffset = Math.min(0, bound.getBounds().getMinX());
        for (int i = 0; i < axes + 1; i++) {
            if (bound.getBounds().getMaxX() - xOffset <= 0.5) xOffset--;
        ...
        double yOffset = 0;
        for (int i = 0; i < axes + 1; i++) {
            if (bound.getBounds().getMaxY() - yOffset <= 0.5) yOffset--;
    ...
    public static Point[] getRayHeads(BufferedImage image, double angle, Point pivot, Polygon bound){
        ...
        if(angle!=90){
            ...
            for (int x = (int) bound.getBounds().getMinX(); x < xUpperBound; x++) {
                ...
                if (GeometricToolBox.isInBound(target, bound) && !isTransparent(image, target) && !isSurrounded(image, target, bound)){
                    maxPoint = target;
    ...
    public static boolean isSurrounded(BufferedImage image, Point point, Polygon bound){
        ...
        boolean valid = GeometricToolBox.isInBound(L, bound) && GeometricToolBox.isInBound(R, bound) && GeometricToolBox.isInBound(U, bound) && GeometricToolBox.isInBound(D, bound);
