...
//            collidable.onCollision(this);
    ...
    private void handleCircleCircleCollision(Collidable collidable) {
        double distance = Utils.findDistance(getAnchor(), collidable.getAnchor());
    ...
    private void handleCirclePolygonCollision(Collidable collidable) {
        if (collidable instanceof FinalPanelModel) {
            Point2D intersection = Utils.findClosestPointOnEdges(getAnchor(), collidable.getEdges());
...
//            System.out.println("BARICAAAAAAAAAAAA");
    ...
    private static Point2D findCircleCircleIntersection(Collidable c1, Collidable c2) {
        ...
        return Utils.findMidPoint(intersection1, intersection2);
    }
...
//    }
    ...
    default Point2D closestPointOnPolygon(Point2D point, Point2D[] vertices){
        ...
        for (int i=0;i<vertices.length;i++){
            ...
            Point2D temp = Utils.getClosestPointOnSegment(vertices[i],vertices[(i+1)%vertices.length],point);
    ...
    default void findSingleIntersectionPoint(Collidable poly2) {
        ...
        for (int i = 0; i < vertices1.length; i++) {
            edges1[i] = Utils.relativeLocation(vertices1[(i + 1) % vertices1.length], vertices1[i]);
        }

        for (int i = 0; i < vertices2.length; i++) {
            edges2[i] = Utils.relativeLocation(vertices2[(i + 1) % vertices2.length], vertices2[i]);
        }
        ...
        if  (intersectionPoints.size() == 2) {
            ...
            if (point1.edge == point2.edge) {
                collisionPointOfPoly1 =
                        Utils.findMidPoint(point1.collision, point2.collision);
                collisionNormalVectorOfPoly1 = Utils.normalizeVector(Utils.perpendicularClockwise(edges1[point1.edge]));
            } else {
                int vertex;
                ...
                double distance1 = Utils.findDistance(point1.collision, vertices1[vertex]);
                double distance2 = Utils.findDistance(point2.collision, vertices1[vertex]);

                if (distance1 < distance2) {
//                    collisionPointOfPoly1 = point2.collision;
                    collisionPointOfPoly1 = vertices1[vertex];

                    collisionNormalVectorOfPoly1 = Utils.normalizeVector(Utils.perpendicularClockwise(vertices1[vertex]));
                } else {
                    collisionPointOfPoly1 = vertices1[vertex];
//                    collisionPointOfPoly1 = point1.collision;
            ...
            if (intersection1.edge == intersection2.edge) {
                collisionPointOfPoly2 = Utils.findMidPoint(intersection1.collision, intersection2.collision);
                int edgeNumber = intersection1.edge;
                Point2D intersectingEdge = Utils.relativeLocation(vertices2[(edgeNumber + 1) % vertices2.length], vertices2[edgeNumber]);
                collisionNormalVectorOfPoly2 = Utils.normalizeVector(Utils.perpendicularClockwise(intersectingEdge));
            } else {
                ...
                double distance1 = Utils.findDistance(intersection1.collision, vertices2[vertex]);
                double distance2 = Utils.findDistance(intersection2.collision, vertices2[vertex]);
                if (distance1 < distance2) {
...
//                    collisionPointOfPoly2 = intersection1.collision;
    ...
    private Point2D handleVertexCollision(Point2D[] vertices, Intersection point1, Intersection point2) {
        if ((point1.edge + 1) % vertices.length == point2.edge) {
            return Utils.findMidPoint(point1.collision, vertices[point2.edge]);
        } else {
            return Utils.findMidPoint(point2.collision, vertices[point1.edge]);
        }
    }
